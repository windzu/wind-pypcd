...
def _validate_bounds(
    min_bounds: Union[Sequence[float], np.ndarray],
    max_bounds: Union[Sequence[float], np.ndarray],
) -> Tuple[np.ndarray, np.ndarray]:
    """验证裁剪边界的有效性并返回 float32 数组"""

    min_arr = np.asarray(min_bounds, dtype=np.float32).reshape(-1)
    max_arr = np.asarray(max_bounds, dtype=np.float32).reshape(-1)

    if min_arr.shape != (3,) or max_arr.shape != (3,):
        raise ValueError("min_bounds 和 max_bounds 必须是长度为 3 的序列")

    if not np.isfinite(min_arr).all() or not np.isfinite(max_arr).all():
        raise ValueError("裁剪范围包含 NaN 或 Inf")

    if np.any(min_arr > max_arr):
        raise ValueError("min_bounds 不能大于 max_bounds")
...
def _apply_transform_to_points(
    ...
    points_homo = np.column_stack(
        [x_valid, y_valid, z_valid, np.ones(len(x_valid), dtype=np.float32)]
    )
...
def fusion_pcd(
    datas: Dict[str, str], calib: Dict[str, np.ndarray], save_path: str
) -> str:
    """根据标定信息合并多个PCD文件
    ...
    if not all(key in calib for key in datas.keys()):
        raise ValueError(
            "All sensors in datas must have corresponding calibration in calib"
        )
...
def fusion_pcd_bytes(
    datas_bytes: Dict[str, bytes],
    calib: Dict[str, np.ndarray],
    compression: str = "binary_compressed",
) -> bytes:
    """根据标定信息合并多个PCD字节数据
    ...
    if not all(key in calib for key in datas_bytes.keys()):
        raise ValueError(
            "All sensors in datas_bytes must have corresponding calibration in calib"
        )
    ...
    for sensor in datas_bytes.keys():
        try:
            transformed_points = transform_pcd_from_bytes(
                datas_bytes[sensor], calib[sensor]
            )
            point_clouds.append(transformed_points)
            print(f"Processed {sensor}: {len(transformed_points)} points")
        except Exception as e:
...
def crop_point_cloud(
    ...
    """裁剪 PointCloud 使其仅保留指定 AABB（轴对齐包围盒）内的点"""

    if not isinstance(pc, pypcd.PointCloud):
        raise ValueError("pc 必须是 PointCloud 对象")

    required_fields = {"x", "y", "z"}
    dtype_fields = set(pc.pc_data.dtype.names or [])
    if not required_fields.issubset(dtype_fields):
        missing = sorted(required_fields.difference(dtype_fields))
        raise ValueError(f"点云缺少必要字段: {missing}")

    min_arr, max_arr = _validate_bounds(min_bounds, max_bounds)

    mask = (
        (pc.pc_data["x"] >= min_arr[0])
        & (pc.pc_data["x"] <= max_arr[0])
        & (pc.pc_data["y"] >= min_arr[1])
        & (pc.pc_data["y"] <= max_arr[1])
        & (pc.pc_data["z"] >= min_arr[2])
        & (pc.pc_data["z"] <= max_arr[2])
    )

    if not mask.any():
        raise ValueError("裁剪结果为空，请检查指定的范围")

    cropped_data = pc.pc_data[mask].copy()

    metadata = pc.get_metadata()
    metadata["points"] = len(cropped_data)
    metadata["width"] = len(cropped_data)
    metadata["height"] = 1

    return pypcd.PointCloud(metadata, cropped_data)
...
def crop_pcd(
    ...
    compression: str = "binary_compressed",
) -> Union[pypcd.PointCloud, str]:
    """裁剪 PCD 文件，返回裁剪后的 PointCloud 或保存路径"""

    if not isinstance(pcd_path, str):
        raise ValueError("pcd_path 必须是字符串")

    if not os.path.exists(pcd_path):
        raise FileNotFoundError(f"PCD 文件不存在: {pcd_path}")

    pc = pypcd.PointCloud.from_path(pcd_path)
    cropped = crop_point_cloud(pc, min_bounds, max_bounds)
    ...
    cropped.save_pcd(output_path, compression=compression)
    return output_path


def crop_pcd_from_bytes(
    pcd_bytes: bytes,
    min_bounds: Union[Sequence[float], np.ndarray],
    max_bounds: Union[Sequence[float], np.ndarray],
    compression: str = "binary_compressed",
) -> bytes:
    """裁剪 PCD 字节数据并返回裁剪后的字节流"""

    if not isinstance(pcd_bytes, bytes):
        raise ValueError("pcd_bytes 必须是 bytes")

    pc = pypcd.PointCloud.from_bytes(pcd_bytes)
    cropped = crop_point_cloud(pc, min_bounds, max_bounds)
    return cropped.to_bytes(compression=compression)
...
if __name__ == "__main__":
    ...
    print("\n=== 轴对齐裁剪示例 ===")
    try:
        bounds_min = (-2.0, -2.0, -1.5)
        bounds_max = (2.0, 2.0, 1.5)
        cropped_output = "test_data/results/front_lidar_cropped.pcd"
        crop_pcd(input_path, bounds_min, bounds_max, cropped_output)
        cropped_pc = pypcd.PointCloud.from_path(cropped_output)
        cropped_points = cropped_pc.get_metadata().get("points", 0)
        print(
            "裁剪后点数:",
            cropped_points,
            ", 输出文件:",
